...
void set_color(unsigned char* framebuffer, TGAColor& color, int x, int y) {
    int i;
...
//绘制三角形
void triangle(Vec4f* pts, IShader& shader, unsigned char* framebuffer, TGAImage& zbuffer) {
    //初始化三角形边界框
    ...
    for (P.x = bboxmin.x; P.x <= bboxmax.x; P.x++) {
        for (P.y = bboxmin.y; P.y <= bboxmax.y; P.y++) {
            ...
            if (!discard) {
                ...
                //为像素设置颜色
                set_color(framebuffer,color,P.x,P.y);
            }
        }
    }
}